...
```bash
# Launch the complete simulation with planner and controller
...
2. **Force Estimator** (force_estimator.hpp): Online disturbance estimation
3. Outputs SO3Command to quadrotor simulator

### Configuration Files

- `planner/plan_manage/config/planning_params.yaml` - Planning weights, system parameters (masses, cable length), search parameters
- `controller/payload_mpc_controller/config/mpc.yaml` - MPC cost matrices, input limits, force estimator settings
...
6. Rebuild the workspace: `catkin_make -DCMAKE_BUILD_TYPE=Release`
...
- **Trajectory Representation**: Uses MINCO (Minimum Control Effort) trajectories, not standard polynomials
...
- `/planning/trajectory` - Published trajectory for controller
- `/so3cmd` - SO(3) control command output
