...
namespace PayloadMPC
{
	...
	nh_.param("warmup_trim", warmup_trim_, 0.0);
	if (warmup_trim_ < 0.0)
	{
		warmup_trim_ = 0.0;
	}
	...
	nh_.param("analytic_target_cycles", analytic_target_cycles_, 4);
	...
	analytic_active_ = warmup_trim_ <= 1e-6;
	analytic_active_start_ = ros::Time::now();
	...
	if (analytic_traj_)
	{
		...
		analytic_start_time_ = ros::Time::now();
		analytic_active_start_ = analytic_start_time_;
		...
		if (std::isfinite(analytic_cycle_time_) && analytic_cycle_time_ > 0.0)
		{
			if (std::isfinite(analytic_total_duration_))
			{
				...
				if (available_int <= 0)
					available_int = 1;
				if (available_int < analytic_target_cycles_)
				{
					ROS_WARN("[MPCctrl] analytic_target_cycles reduced from %d to %d due to finite duration.", analytic_target_cycles_, available_int);
					analytic_target_cycles_ = available_int;
				}
			}
...
{
	// 在仿真 + 分析轨迹模式下，前几个秒使用 L-BFGS，等系统进入稳定轨迹后再切换到 PINN。
	// 这样可以避免 PINN 在“起步瞬间”（训练数据未覆盖的状态）给出不可靠的力估计，导致姿态/偏航跑飞。
	const double kPinnActivationDelay = 0.0; // [s] 延迟启用 PINN 的时间窗口
	bool force_lbfgs_due_to_delay = false;

	if (params_.use_simulation_ && analytic_traj_)
	{
		const double traj_time = (ros::Time::now() - analytic_start_time_).toSec();
		if (traj_time < kPinnActivationDelay)
		{
			force_lbfgs_due_to_delay = true;
		}
	}

	if (force_lbfgs_due_to_delay)
	{
		if (lbfgs_estimator_ && force_estimator_ != lbfgs_estimator_.get())
		{
	...
	*/
	void MPCFSM::CMD_CTRL_process()
	{
		...
		case ANALYTIC:
		{
			...
			update_hover_pose();
			double traj_time_raw = (now_time - analytic_start_time_).toSec();
			if (traj_time_raw < 0.0)
			{
				traj_time_raw = 0.0;
				resetAnalyticStart(now_time);
			}

				// Warmup handling: keep tracking the trajectory, but only start analytics/export after warmup_trim_.
				if (!analytic_active_ && traj_time_raw >= warmup_trim_)
				{
					analytic_active_ = true;
					analytic_active_start_ = now_time;
					analytic_rmse_sum_quad_ = 0.0;
					analytic_rmse_sum_payload_ = 0.0;
					analytic_rmse_samples_ = 0;
					analytic_cycles_completed_ = 0;
				...
				const double traj_time_eff = analytic_active_ ? std::max(0.0, traj_time_raw - warmup_trim_) : 0.0;

			auto sampler = [this](double query_t) {
				return analytic_traj_->sample(query_t);
			};

		controller_.setAnalyticReference(sampler, traj_time_raw, hover_yaw_);
		controller_.execMPC(est_state_, mpc_predicted_states_, mpc_predicted_inputs_);
		...
		if (std::isfinite(analytic_cycle_time_) && analytic_cycle_time_ > 0.0)
		{
			while (!analytic_rmse_reported_ && analytic_cycles_completed_ < analytic_target_cycles_ && traj_time_eff >= analytic_next_cycle_time_)
			{
				analytic_cycles_completed_++;
				analytic_next_cycle_time_ += analytic_cycle_time_;
			}
		}
		else if (std::isfinite(analytic_total_duration_) && traj_time_eff >= analytic_total_duration_)
		{
			analytic_cycles_completed_ = analytic_target_cycles_;
		}
	if (!analytic_rmse_reported_ && analytic_rmse_samples_ > 0 && (analytic_cycles_completed_ >= analytic_target_cycles_ || (std::isfinite(analytic_total_duration_) && traj_time_eff >= analytic_total_duration_)))
	{
		...
		if (std::isfinite(duration) && traj_time_eff >= duration)
		{
			ROS_INFO_THROTTLE(5.0, "[MPCctrl] ANALYTIC trajectory reached duration %.2f s, holding terminal state.", duration);
			}
			else
			{
				ROS_INFO_THROTTLE(2.0, "[MPCctrl] ANALYTIC trajectory tracking t=%.2f s", traj_time_eff);
			}
		}
		}
		break;
