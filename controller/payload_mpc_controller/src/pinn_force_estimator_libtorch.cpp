...
namespace PayloadMPC
{
  ...
  bool PinnForceEstimator::runInference(Eigen::Vector3d &fl, Eigen::Vector3d &fq, std::string &reason)
  {
    try
    {
      ...
      // NOTE: The current training pipeline (estimation.force_estimation)
      // produces forces directly in物理单位[N]，仅对输入特征做了 MinMax → [-1,1] 归一化。
      // TorchScript 模型输出同样是未归一化的力，因此这里不再对输出应用
      // normalizer_stats.json 中的 MinMax 反归一化，而是直接使用网络输出。
      // 这样可以保证与 Python 端的推理结果一致，避免在 C++ 端重复缩放导致的系统性偏差。
      fq << static_cast<double>(ptr[0]),
          static_cast<double>(ptr[1]),
          static_cast<double>(ptr[2]);
      fl << static_cast<double>(ptr[3]),
          static_cast<double>(ptr[4]),
          static_cast<double>(ptr[5]);
      return true;
    }
    catch (const c10::Error &e)
    {
  ...
  {
    // Only report operational when we have everything loaded AND at least one valid system state.
    return use_estimator_ && model_ready_ && normalizer_ready_ && state_ready_;
  }
