...
如果 CODE_GEN 为真，系统将被编译成一个实时迭代的优化问题，并生成所有用于在线运行的代码。约束条件和参考结构会被使用，但其值将在运行时设置。

如果 CODE_GEN 为假，系统将被编译成一个独立的优化程序，并在执行时求解。参考值和约束条件必须在此处设置。
*/
#define CODE_GEN true

// 如果你想使用自己的参数，请运行程序并指定你自己的参数文件路径。例如：
...
int main(int argc, char **argv)
{
  ...
  // cable_x, cable_y, cable_z 分别代表电缆在 x、y、z 轴上的分量，是微分状态变量。
  // 它们描述了电缆相对于某个参考系在各个坐标轴方向上的位置或偏移。
  DifferentialState cable_x, cable_y, cable_z;
  // dcable_x, dcable_y, dcable_z 分别代表电缆在 x、y、z 轴上分量的变化率，是微分状态变量。
  // 即电缆在各个坐标轴方向上位置或偏移的变化速度。
  ...
  #if CODE_GEN
  OnlineData Mq;        // 四旋翼质量 [kg]，在代码生成模式下作为在线数据，可在运行时动态设置其值。
  OnlineData Ml;        // 负载质量 [kg]，在代码生成模式下作为在线数据，可在运行时动态设置其值。
  OnlineData l_length;  // 电缆长度 [m]，在代码生成模式下作为在线数据，可在运行时动态设置其值。
  // OnlineData fl_x, fl_y, fl_z; // 外部力 [N]

  #if (USE_CONSTANT_MOMENT==1)
  OnlineData M_x, M_y, M_z; // 恒定力矩 [N*m]，在 USE_CONSTANT_MOMENT 为 1 时，作为在线数据，可在运行时动态设置其值。
  IntermediateState fl_x, fl_y, fl_z; // 中间状态变量，表示作用在负载上的外部力分量，其值由恒定力矩计算得出。
  OnlineData fq_x, fq_y, fq_z;  // 作用在四旋翼上的外部力 [N]，在 USE_CONSTANT_MOMENT 为 1 时，作为在线数据，可在运行时动态设置其值。
  #elif (USE_CONSTANT_MOMENT==2)
  OnlineData fl_x, fl_y, fl_z;  // 作用在四旋翼上的外部力 [N]，在 USE_CONSTANT_MOMENT 为 2 时，作为在线数据，可在运行时动态设置其值。
  OnlineData M_x, M_y, M_z; // 恒定力矩 [N*m]，在 USE_CONSTANT_MOMENT 为 2 时，作为在线数据，可在运行时动态设置其值。
  IntermediateState fq_x, fq_y, fq_z; // 中间状态变量，表示作用在四旋翼上的外部力分量，其值由恒定力矩计算得出。
  #else
  OnlineData fl_x, fl_y, fl_z;  // 作用在负载上的外部力 [N]，在 USE_CONSTANT_MOMENT 不为 1 和 2 时，作为在线数据，可在运行时动态设置其值。
  OnlineData fq_x, fq_y, fq_z; // 作用在四旋翼上的外部力 [N]，在 USE_CONSTANT_MOMENT 不为 1 和 2 时，作为在线数据，可在运行时动态设置其值。
  #endif
  ...
  q_sqr = q_w*q_w - q_x*q_x- q_y*q_y + q_z*q_z;            //C
  qwqy_plus_qxqz = 2*(q_w*q_y + q_x*q_z);                 //A
  qwqx_minus_qyqz = 2*(q_w*q_x - q_y*q_z);                //B
  ...
  //机体平动
  //▲_x = (fl_x - (Ml*(fq_x + T*A))/Mq)   
  //▲_y = (fl_y - (Ml*(fq_y - T*B))/Mq)    
  // ▲_z = (fl_z - (Ml*(fq_z + T*C))/Mq))
  //dot(v_x) == (fq_x + T*A)/Mq + (ρ_x*((ρ_x*▲_x) + ρ_y*▲_y + ρ_z*▲_z） + Ml*ρ_x*l_length*(dcable_sqr))/(Ml + Mq);
  ...
  f << dot(v_z) == - g_z + (fq_z + T*(q_sqr))/Mq + (cable_z*(cable_x*(fl_x - (Ml*(fq_x + T*(qwqy_plus_qxqz)))/Mq) + cable_y*(fl_y - (Ml*(fq_y - T*(qwqx_minus_qyqz)))/Mq) + cable_z*(fl_z - (Ml*(fq_z + T*(q_sqr)))/Mq)) + Ml*cable_z*l_length*(dcable_sqr))/(Ml + Mq) ;
  ...
  //载荷平动
  //Γ_x = (fq_x + T*A - (Mq*fl_x)/Ml)
  //Γ_y = (- fq_y + T*B  + (Mq*fl_y)/Ml)
  //Γ_z = (fq_z + T*C - (Mq*fl_z)/Ml)
  //dot(payload_vx) == fl_x/Ml - (ρ_x*(ρ_y*Γ_y - ρ_z*Γ_z - ρ_x*Γ_x + Mq*l_length*(dcable_sqr)))/(Ml + Mq);
  ...
  //缆绳方向动力学
  //ξ_x = ((fq_x + T*A)/Mq - fl_x/Ml)
  //ξ_y = ((fq_y - T*B)/Mq - fl_y/Ml)
  //ξ_z = (fl_z/Ml - (fq_z + T*C)/Mq)
  //dot(dcable_x) == - (ρ_y*(ρ_y*ξ_x - ρ_x*ξ_y) + ρ_z*(ρ_x*ξ_z + ρ_z*ξ_x))/l_length - ρ_x*(dcable_sqr);
  ...
  //dot(dcable_y) == (ρ_y*(ρ_y*ξ_x - ρ_x*ξ_y) - ρ_z*(ρ_y*ξ_z + ρ_z*ξ_y))/l_length - ρ_y*(dcable_sqr);
  ...
  // Running cost vector consists of all states and inputs.运行成本向量由所有状态和输入组成。
  h << p_x << p_y << p_z  //3
    << q_w << q_x << q_y << q_z  //7 
    << v_x << v_y << v_z //10
  ...
  // End cost vector consists of all states (no inputs at last state).结束成本向量由所有状态组成（最后一个状态没有输入）。
  hN << p_x << p_y << p_z
     << q_w << q_x << q_y << q_z
     << v_x << v_y << v_z
  ...
  //  Running cost weight matrix.运行成本权重矩阵。
  DMatrix Q(h.getDim(), h.getDim());
  Q.setIdentity();

  // Set a reference for the analysis (if CODE_GEN is false).分析的参考值（如果CODE_GEN为false）。
  ...
  DVector rN(hN.getDim()); // End cost reference.结束成本参考值。
  rN.setZero();
  rN(0) = r(0);
  rN(1) = r(1);
  rN(2) = r(2);
  ...
  // DEFINE AN OPTIMAL CONTROL PROBLEM:定义一个最优控制问题。
  // ----------------------------------
  OCP ocp(0.0, dt * N, N);
  #if (!CODE_GEN)
  {
