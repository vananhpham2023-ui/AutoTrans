//相比于quadrotor_payload_mpc_with_ext_force.cpp，这个代码文件引入模式切换参数S（0-1连续值）
//S参数作为混合模式开关，允许MPC控制器在运行时通过调整该参数值（0或1）来切换不同的动力学模型配置，实现四旋翼-负载系统的混合控制策略。
...
// 此代码行的作用是从配置文件中读取 "step_N" 字段的值，并将其转换为整数类型赋值给变量 N。
// 如果配置文件中没有该字段，则使用默认值 20。N 代表节点的数量。
  ...
  f << dot(v_x) == (fq_x + T*(qwqy_plus_qxqz))/Mq + S*(cable_x*(cable_x*(fl_x - (Ml*(fq_x + T*(qwqy_plus_qxqz)))/Mq) + cable_y*(fl_y - (Ml*(fq_y - T*(qwqx_minus_qyqz)))/Mq) + cable_z*(fl_z - (Ml*(fq_z + T*(q_sqr)))/Mq)) + Ml*cable_x*l_length*(dcable_sqr))/(Ml + Mq);

  f << dot(v_y) == (fq_y - T*(qwqx_minus_qyqz))/Mq + S*(cable_y*(cable_x*(fl_x - (Ml*(fq_x + T*(qwqy_plus_qxqz)))/Mq) + cable_y*(fl_y - (Ml*(fq_y - T*(qwqx_minus_qyqz)))/Mq) + cable_z*(fl_z - (Ml*(fq_z + T*(q_sqr)))/Mq)) + Ml*cable_y*l_length*(dcable_sqr))/(Ml + Mq);

  f << dot(v_z) == S*(cable_z*(cable_x*(fl_x - (Ml*(fq_x + T*(qwqy_plus_qxqz)))/Mq) + cable_y*(fl_y - (Ml*(fq_y - T*(qwqx_minus_qyqz)))/Mq) + cable_z*(fl_z - (Ml*(fq_z + T*(q_sqr)))/Mq)) + Ml*cable_z*l_length*(dcable_sqr))/(Ml + Mq) - g_z + (fq_z + T*(q_sqr))/Mq;

  f << dot(payload_x) == payload_vx;
  f << dot(payload_y) == payload_vy;
  f << dot(payload_z) == payload_vz;

  f << dot(payload_vx) == fl_x/Ml - S*(cable_x*(cable_y*(T*(qwqx_minus_qyqz) - fq_y + (Mq*fl_y)/Ml) - cable_z*(fq_z + T*(q_sqr) - (Mq*fl_z)/Ml) - cable_x*(fq_x + T*(qwqy_plus_qxqz) - (Mq*fl_x)/Ml) + Mq*l_length*(dcable_sqr)))/(Ml + Mq);
  f << dot(payload_vy) == fl_y/Ml - S*(cable_y*(cable_y*(T*(qwqx_minus_qyqz) - fq_y + (Mq*fl_y)/Ml) - cable_z*(fq_z + T*(q_sqr) - (Mq*fl_z)/Ml) - cable_x*(fq_x + T*(qwqy_plus_qxqz) - (Mq*fl_x)/Ml) + Mq*l_length*(dcable_sqr)))/(Ml + Mq);
  f << dot(payload_vz) == fl_z/Ml - g_z - S*(cable_z*(cable_y*(T*(qwqx_minus_qyqz) - fq_y + (Mq*fl_y)/Ml) - cable_z*(fq_z + T*(q_sqr) - (Mq*fl_z)/Ml) - cable_x*(fq_x + T*(qwqy_plus_qxqz) - (Mq*fl_x)/Ml) + Mq*l_length*(dcable_sqr)))/(Ml + Mq);

  f << dot(cable_x) == dcable_x;
  f << dot(cable_y) == dcable_y;
  f << dot(cable_z) == dcable_z;

  f << dot(dcable_x) ==  S*(- (cable_y*(cable_y*((fq_x + T*(qwqy_plus_qxqz))/Mq - fl_x/Ml) - cable_x*((fq_y - T*(qwqx_minus_qyqz))/Mq - fl_y/Ml)) + cable_z*(cable_x*(fl_z/Ml - (fq_z + T*(q_sqr))/Mq) + cable_z*((fq_x + T*(qwqy_plus_qxqz))/Mq - fl_x/Ml)))/l_length - cable_x*(dcable_sqr)) +(1-S)* (fl_x/Ml - (fq_x + T*(qwqy_plus_qxqz))/Mq);

  f << dot(dcable_y) == S*((cable_x*(cable_y*((fq_x + T*(qwqy_plus_qxqz))/Mq - fl_x/Ml) - cable_x*((fq_y - T*(qwqx_minus_qyqz))/Mq - fl_y/Ml)) - cable_z*(cable_y*(fl_z/Ml - (fq_z + T*(q_sqr))/Mq) + cable_z*((fq_y - T*(qwqx_minus_qyqz))/Mq - fl_y/Ml)))/l_length - cable_y*(dcable_sqr)) + (1-S)* (fl_y/Ml - (fq_y - T*(qwqx_minus_qyqz))/Mq);

  f << dot(dcable_z) == S*((cable_x*(cable_x*(fl_z/Ml - (fq_z + T*(q_sqr))/Mq) + cable_z*((fq_x + T*(qwqy_plus_qxqz))/Mq - fl_x/Ml)) + cable_y*(cable_y*(fl_z/Ml - (fq_z + T*(q_sqr))/Mq) + cable_z*((fq_y - T*(qwqx_minus_qyqz))/Mq - fl_y/Ml)))/l_length - cable_z*(dcable_sqr)) + (1-S)*( fl_z/Ml - (fq_z + T*(q_sqr))/Mq);
  ...
  #else
  {
    ...
    if (mpc.exportCode("hybird_quadrotor_payload_mpc") != SUCCESSFUL_RETURN)
      exit(EXIT_FAILURE);
