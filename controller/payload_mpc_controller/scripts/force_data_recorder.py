...
class ForceDataRecorder:
    ...
    def __init__(self) -> None:
        ...
        self.warmup_trim = max(0.0, float(rospy.get_param("~warmup_trim", 0.0)))
        self._wind_enabled = bool(self.wind_topic)

        self._samples: List[Dict[str, np.ndarray]] = []
        self._lock = threading.Lock()
        self._start_time: Optional[float] = None
        self._logging_start_time: Optional[float] = None
        self._warmup_done = self.warmup_trim <= 1e-6
    ...
    def _on_force_pair(self, true_msg: AccelStamped, est_msg: AccelStamped) -> None:
        ...
        with self._lock:
            ...
            if not self._warmup_done:
                elapsed_warmup = timestamp - self._start_time
                if elapsed_warmup < self.warmup_trim:
                    return
                self._warmup_done = True
                self._logging_start_time = timestamp
                self._start_time = timestamp
                self._warmup_logged = True
                rospy.loginfo("[force_data_recorder] Warmup finished after %.3f s, starting force logging.", elapsed_warmup)
                rospy.loginfo("[force_data_recorder] Warmup finished after %.3f s, starting force logging.", elapsed_warmup)
            if self._logging_start_time is None:
                self._logging_start_time = self._start_time
            start_ref = self._logging_start_time or self._start_time
