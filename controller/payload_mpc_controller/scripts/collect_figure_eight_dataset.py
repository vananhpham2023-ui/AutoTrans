#!/usr/bin/env python3
"""
Automate data collection and cleaning for two special figure-eight scenarios used
for PINN training. The script sequentially launches each scenario, records a fixed
...
Current scenarios（与 TASK3/TASK9 对应）：
- figure_eight_gradual_cycles    （gradual 周期开关风场）
- figure_eight_gust_event        （多阶段 gust_event 阵风 + Dryden）
...
SCENARIOS = [
    {
        # gradual 周期开关风场（独立 gradual wind，配合 Dryden 湍流）
        "name": "figure_eight_gradual_cycles",
        "type": "gradual",
    ...
    {
        # 多阶段 gust_event 阵风 + Dryden（三轴湍流）
        "name": "figure_eight_gust_event",
        "type": "gust_event",
        "wind_sigma": [3.256, 3.256, 2.31],
        "wind_length": [420.0, 420.0, 300.0],
        "wind_override_yaml": "{gust_event: {axis: [1.0,1.0,0.0], magnitude: 5, start_time: 8.0, hold_duration: 20.0, rise_time: 1.3, fall_time: 1.3, axis2: [1.0,1.0,0.0], magnitude2: 9.5, hold_duration2: 20.0, rise_time2: 1.3, fall_time2: 1.3, multi_stage: true}}",
    },
]
...
def collect_for_scenario(
    ...
    scenario_type = str(scenario.get("type", "composite"))
    if scenario_type == "gradual":
        # gradual 周期开关风场，配合 Dryden 湍流
        wind_const = scenario["wind_constant_velocity"]
        base_launch = (
            ...
            "wind_type:=gradual "
            f"wind_constant_velocity:='{wind_const}' "
            f"wind_dryden_sigma:='{scenario['wind_sigma']}' "
            f"wind_dryden_length_scale:='{scenario['wind_length']}' "
            "wind_window_enabled:=true wind_window_auto_from_cycle:=true "
            "wind_window_skip_cycles:=1 wind_window_active_cycles:=8 "
            "wind_gradual_cycle_profile_enabled:=true "
            "wind_gradual_ramp_up_time:=20 wind_gradual_ramp_down_time:=20 "
            f"run_label:={scenario['name']} warmup_trim:={args.warmup}"
        )
    elif scenario_type == "gust_event":
        # 多阶段 gust_event 阵风 + Dryden（三轴湍流）
        base_launch = (
            "roslaunch payload_planner controller_only.launch "
            "trajectory_mode:=figure_eight figure_radius:=3.0 figure_omega:=0.8 "
            ...
            f"wind_dryden_sigma:='{scenario['wind_sigma']}' "
            f"wind_dryden_length_scale:='{scenario['wind_length']}' "
            f"wind_override_yaml:='{scenario['wind_override_yaml']}' "
            f"run_label:={scenario['name']} warmup_trim:={args.warmup}"
        )
    else:
        # 默认：复合风 + 不同湍流强度
