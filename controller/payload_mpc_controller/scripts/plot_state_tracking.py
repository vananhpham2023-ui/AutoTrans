...
def _downsample_and_smooth(
    ...
    value_series: Sequence[Sequence[float]],
...
def render_tracking_plot(
    ...
    fig, axes = plt.subplots(len(series), 1, figsize=(12, 7), sharex=True)
    if len(series) == 1:
        axes = [axes]

    prepared = []
    for axis_label, samples in series.items():
        t_ds, (ref_ds, act_ds) = _downsample_and_smooth(times, [samples["reference"], samples["actual"]])
        prepared.append((axis_label, t_ds, ref_ds, act_ds))

    for axis, (axis_label, t_ds, ref_ds, act_ds) in zip(axes, prepared):
        axis.plot(t_ds, ref_ds, label="Reference", color=LINE_COLORS["reference"], linewidth=1.0)
        axis.plot(t_ds, act_ds, label="Actual", color=LINE_COLORS["actual"], linewidth=0.9, linestyle="--")
...
def render_error_panels(
    ...
    for label, values in label_value_pairs:
        t_ds, (err_ds,) = _downsample_and_smooth(times, [values])
        prepared.append((label, t_ds, err_ds))

    for idx, (axis, (label, t_ds, err_ds)) in enumerate(zip(axes, prepared)):
        axis.plot(t_ds, err_ds, color=ERROR_COLOR, linewidth=1.0)
