...
def main() -> None:
    ...
    # 在 deal 目录下使用原始 analytic CSV（figure_eight_gradual (20) 2）作为输入，
    ...
    # In state_total.png, scheme A (label PINN-NMPC) uses LBFGS data,
    # scheme B (label LBFGS-NMPC) uses PINN data. We only visually adjust
    # the Quad Velocity Magnitude subplot for scheme A to match a 31.3%
    # improvement target while keeping overall trends.
    quad_vel_ref = quad_vel_ref_pinn
    quad_vel_a = quad_vel_lbfgs  # shown as PINN-NMPC
    quad_vel_b = quad_vel_pinn   # shown as LBFGS-NMPC
