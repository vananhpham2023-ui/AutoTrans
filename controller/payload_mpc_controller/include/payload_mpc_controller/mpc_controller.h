...
namespace PayloadMPC
{
  ...
  public:
    ...
    void setAnalyticReference(const std::function<Eigen::Matrix<double, 3, 6>(double)> &trajectory_fn,
                              double tstart,
                              double start_yaw);
    ...
    /**
    * @brief 设置外部作用在负载和四旋翼上的力，并更新类内部存储的外部力变量
    * 
    * 该函数将外部作用在负载和四旋翼上的力传递给 MpcWrapper 对象，
    * 同时将传入的力分别赋值给类内部对应的存储变量。
    * 
    * @param fl 外部作用在负载上的力，使用 Eigen::Vector3d 类型表示
    * @param fq 外部作用在四旋翼上的力，使用 Eigen::Vector3d 类型表示
    */
    void setExternalForce(const Eigen::Ref<const Eigen::Vector3d>& fl, const Eigen::Ref<const Eigen::Vector3d>& fq)    
    {
        mpc_wrapper_.setExternalForce(fl, fq);    
        fq_ = fq;  
        fl_ = fl;  
    }
