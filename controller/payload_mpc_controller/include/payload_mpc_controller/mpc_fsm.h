...
namespace PayloadMPC
{
	...
	public:
		...
		enum Exec_Traj_State_t
		{
			...
			ANALYTIC = 13,	// execute analytic closed-form trajectory
		};
		...
		void setAnalyticTrajectory(std::unique_ptr<AnalyticTrajectory> trajectory);
		AnalyticTrajectory *analytic_trajectory() const { return analytic_traj_.get(); }
		void resetAnalyticStart(const ros::Time &stamp);
	...
	private:
		...
		double warmup_trim_{0.0};			   // seconds to ignore before starting analytics/export
		bool analytic_active_{true};		   // false during warmup
		ros::Time analytic_active_start_;	   // base stamp for analytics/export after warmup
