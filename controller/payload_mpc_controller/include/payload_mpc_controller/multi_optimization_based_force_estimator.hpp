...
namespace PayloadMPC
{
  ...
  class MultiOptForceEstimator
  {
    ...
    static double cost_function(void *instance, const Eigen::VectorXd &x, Eigen::VectorXd &g)
    {
      ...
      Eigen::Vector3d fl_avg = fl_array.rowwise().sum() / double(estimator.param_num_);
      Eigen::Matrix3Xd fl_diff = fl_array;
      fl_diff.colwise() -= fl_avg;
      cost += estimator.var_weight_ * fl_diff.squaredNorm();

      Eigen::Matrix3Xd fl_var_grad = fl_diff;
      Eigen::Vector3d temp_fl = fl_diff.rowwise().sum() / double(estimator.param_num_);
      fl_var_grad.colwise() -= temp_fl;
      fl_gradient += estimator.var_weight_ * 2.0 * fl_var_grad;
  ...
  public:
    ...
    // sample_freq_fq_ 表示四旋翼力估计值（fq）滤波器的采样频率，默认初始化为 333.333Hz。
    double sample_freq_fq_{333.333};  
    // sample_freq_fl_ 表示负载力估计值（fl）滤波器的采样频率，默认初始化为 333.333Hz。
    double sample_freq_fl_{333.333};  
    // cutoff_freq_fq_ 表示四旋翼力估计值（fq）滤波器的截止频率，默认初始化为 100.0Hz。
    double cutoff_freq_fq_{100.0};
    // cutoff_freq_fl_ 表示负载力估计值（fl）滤波器的截止频率，默认初始化为 100.0Hz。
    double cutoff_freq_fl_{100.0};
  ...
  public:
    void init(MpcParams &params)
    {
      ...
      // 初始化力估计器的滤波器
      fq_filter_.set_cutoff_frequency(sample_freq_fq_, cutoff_freq_fq_);// 设置fq滤波器的截止频率(采样频率, 截止频率)
      fl_filter_.set_cutoff_frequency(sample_freq_fl_, cutoff_freq_fl_);// 设置fl滤波器的截止频率(采样频率, 截止频率)  
      fq_filter_.reset(Eigen::Vector3d::Zero());// 重置fq滤波器状态为0向量
      fl_filter_.reset(Eigen::Vector3d::Zero());// 重置fl滤波器状态为0向量


      queue_size_ = params.force_estimator_param_.max_queue;  // 从参数中获取队列最大长度
      opt_variable.resize(2 * 3 * (queue_size_ + 1));  // 初始化优化变量，为fq和fl各分配3*queue_size_空间，额外+1作为缓冲
    ...
    void setSystemState(const Eigen::Vector3d &quad_acc_body, const Eigen::Quaterniond &Rotwb, const Eigen::Vector3d &load_acc_body, const Eigen::Quaterniond &load_Rotwb, const Eigen::Vector3d cable, const Eigen::Vector4d &Rpm)
    {
      ...
      // 此代码块的作用是确保状态缓冲区 state_buffer 的大小不超过预设的队列最大长度 queue_size_。
      // 当 state_buffer 的元素数量超过 queue_size_ 时，会不断从队列头部移除元素，直到满足条件。
    ...
    /**
     * @brief 基于L-BFGS优化的力估计核心计算函数
     * 
     * @param[out] fl  负载力估计值输出向量（三维）
     * @param[out] fq  四旋翼力估计值输出向量（三维）
     * 
     * @note 功能流程：
     * 1. 检查力估计器使能状态
     * 2. 当状态缓冲区数据不足（≤2）时输出零向量
     * 3. 调整优化变量维度并进行数值稳定性检查
     * 4. 使用L-BFGS算法进行带约束的优化计算
     * 5. 处理优化结果边界条件（力矢量幅值限制）
     * 6. 应用低通滤波器平滑输出
     * 
     * @note 关键参数：
     * - 使用lbfgs::lbfgs_optimize进行优化（最大迭代500次）
     * - 力矢量最大幅值限制由max_force_参数控制
     * - 滤波器参数通过sample_freq_*和cutoff_freq_*配置
     * 
