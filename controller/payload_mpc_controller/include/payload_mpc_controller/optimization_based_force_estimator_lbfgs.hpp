...
namespace PayloadMPC
{
  ...
  class OptForceEstimator : public BaseForceEstimator
  {
    ...
    /**
     * @brief 基于L-BFGS优化的代价函数核心实现
     * 
     * @param[in] instance 估计器实例指针（需转换为OptForceEstimator类型）
     * @param[in] x 优化变量矩阵，结构为：
     *              - 前3维：四旋翼力估计值(fq)
     *              - 后3维：负载力估计值(fl)
     * @param[out] g 梯度向量，维度与x相同，结构对应：
     *               - 前3维：fq梯度分量
     *               - 后3维：fl梯度分量
     * 
     * @return double 当前状态下的总代价成本
     *
     * @note 核心功能流程：
     * 1. 初始化梯度向量为零
     * 2. 遍历状态缓冲区中的历史状态数据
     * 3. 根据USE_CONSTANT_MOMENT标志选择力矩约束模式：
     *    - 模式1：约束负载力矩（fl与cable方向正交）
     *    - 模式2：约束四旋翼力矩（fq与cable方向正交） 
     *    - 默认模式：无额外约束
     * 4. 计算以下残差项：
     *    - r1：总力平衡残差（fl + fq = total_force）
     *    - r2：四旋翼力矩平衡残差（基于C参数）
     * 5. 应用Huber损失函数处理残差
     * 6. 累积各状态数据的梯度分量
     *
     * @warning 重要说明：
     * - 状态缓冲区(state_buffer)应包含连续时序数据
     * - 梯度计算需考虑不同约束模式的雅可比转换
     * - 各残差项的Huber损失参数需与物理量纲匹配
     *
     * @remark 典型应用场景：
     * 作为L-BFGS优化器的目标函数，实时求解最优力估计值
     */
  ...
  public:
    ...
    /**
     * @brief 基于L-BFGS优化的实时力估计核心计算函数
     * 
     * @param[out] fl  输出负载力估计值（三维向量，单位：牛顿）
     * @param[out] fq  输出四旋翼力估计值（三维向量，单位：牛顿）
     * 
     * @note 功能流程：
     * 1. 检查力估计器使能状态（use_force_estimator_）
     * 2. 当状态缓冲区数据不足（≤2帧）时输出零向量
     * 3. 配置L-BFGS优化参数（内存大小32，最大迭代500次）
     * 4. 调用lbfgs_optimize进行非线性优化计算
     * 5. 处理优化结果边界条件（力矢量幅值限制）
     * 6. 应用低通滤波器（截止频率由cutoff_freq_*参数控制）
     * 
     * @note 关键特性：
     * - 使用Huber损失函数处理异常值（在cost_function中实现）
     * - 优化变量包含当前和历史状态（通过state_buffer维护）
     * - 支持恒定力矩约束模式（USE_CONSTANT_MOMENT_参数控制）
     * 
     * @warning 重要说明：
     * - 需要保证state_buffer已包含足够状态数据（建议≥3帧）
     * - 最大力值由max_force_参数控制（默认配置在MpcParams中）
     * - 滤波器参数（采样/截止频率）需与系统实际频率匹配
     * 
     * @remark 典型调用场景：
